	...
	void Update () {
        ...
        switch( flickeringLightStyle )
        {
            ...
            case flickerinLightStyles.CampFire:
                ...
                if( campfireMethod == campfireMethods.Intensity || campfireMethod == campfireMethods.Both )
                {
                    ...
                    if( campfireIntesityStyle == campfireIntesityStyles.Sine )
                    {
                        ...
                        // Base + Values
					((Light) GetComponent<Light>()).intensity = CampfireIntensityBaseValue + ( ( Mathf.Sin( CampfireSineCycleIntensity * Mathf.Deg2Rad ) * ( CampfireIntensityFlickerValue / 2.0f ) ) + ( CampfireIntensityFlickerValue / 2.0f ) );
                    }
				else ((Light) GetComponent<Light>()).intensity = CampfireIntensityBaseValue + Random.Range( 0.0f, CampfireIntensityFlickerValue );
                }
                ...
                if( campfireMethod == campfireMethods.Range || campfireMethod == campfireMethods.Both )
                {
                    ...
                    if( campfireRangeStyle == campfireRangeStyles.Sine )
                    {
                        ...
                        // Base + Values
					((Light) GetComponent<Light>()).range = CampfireRangeBaseValue + ( ( Mathf.Sin( CampfireSineCycleRange * Mathf.Deg2Rad ) * ( CampfireSineCycleRange / 2.0f ) ) + ( CampfireSineCycleRange / 2.0f ) );
                    }
				else ((Light) GetComponent<Light>()).range = CampfireRangeBaseValue + Random.Range( 0.0f, CampfireRangeFlickerValue );
                }
                break;
            ...
            case flickerinLightStyles.Fluorescent:
                ...
                {
				((Light) GetComponent<Light>()).intensity = FluorescentFlickerMin;
			...
			else ((Light) GetComponent<Light>()).intensity = FluorescentFlickerMax;
                break;
